...
public class RecipeController {
    ...
    public ResponseEntity<ApiResponse<RecipeResponseDTO>> createRecipe(
        ...
        if (user.getRole() == null || (!user.getRole().name().equals("CHEF") && !user.getRole().name().equals("ADMIN"))) {
            return new ResponseEntity<>(new ApiResponse<>("Only chefs can post recipes", null, HttpStatus.FORBIDDEN.value()), HttpStatus.FORBIDDEN);
        }
